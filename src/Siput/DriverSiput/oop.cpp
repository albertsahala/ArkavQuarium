...
std::set<double> posPressedX;
std::set<double> posPressedY;

void handle_input() {
    SDL_Event e;
    if (!tappedKeys.empty()) tappedKeys.clear();
    if (!posPressedX.empty()) posPressedX.clear();
    if (!posPressedY.empty()) posPressedY.clear();
    while( SDL_PollEvent( &e ) != 0 )
    {
        if ( e.type == SDL_QUIT ) {
            ...
            posPressedX.insert((double) x);
            posPressedY.insert((double) y);
            cout << x << " " << y << endl;
        }
    }
}
...
const std::set<double>& getPosPressedX() {
    return posPressedX;
}

const std::set<double>& getPosPressedY() {
    return posPressedY;
}