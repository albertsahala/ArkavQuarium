...
Ikan::Ikan() : Akuarium (100, 100), Interface ("ikan.png") {
}

/* Konstruktor untuk menentukan posisi ikan ketika pertama kali dibangkitkan*/
Ikan::Ikan(double _x, double _y, double nowTime) : Akuarium (_x, _y), Interface("ikan.png") {
	level = BabyFish;
	hunger = false;
	timeHungry = nowTime;
	fishPopulation++;
...
void Ikan::Move(double sec_since_last) {
	...
	if (hunger) {
		...
		if ((newPosX > 39) && (newPosX < SCREEN_WIDTH - 39) && (newPosY > 154) && (newPosY < SCREEN_HEIGHT - 139)) {
