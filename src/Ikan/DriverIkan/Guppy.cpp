...
Guppy::Guppy() : Ikan (0, 0, 0, "guppy") {
	populationNumber++;
	lastCoinDrop = CoinOut_Time_Guppy;
}

Guppy::Guppy(double _x, double _y) : Ikan (_x, _y, FreeHungry_Time_Guppy, "guppy") {
	populationNumber++;
	lastCoinDrop = CoinOut_Time_Guppy;
}
...
string Guppy::getImageFile() const {
    ...
    switch (level) {
        case BabyFish : return "guppy";
        case TeenFish : return "guppy1";
        default : return"guppy2";
    }
}
